...
def initialize_database():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS pills
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, hour INTEGER, minute INTEGER, am_pm TEXT, bin_number TEXT)''')
    conn.commit()
    conn.close()

# Add a new pill to the schedule
def add_pill(name, hour, am_pm):
    if len(get_all_pills()) >= 30:
        raise ValueError("Maximum limit of 30 pills reached.")
    else:
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("INSERT INTO pills (name, hour, am_pm, bin_number) VALUES (?, ?, ?, ?)", (name, hour, minute, am_pm))
        conn.commit()
        conn.close()
...
def add_pill():
    if request.method == 'POST':
        ...
        try:
            add_pill(name, hour, am_pm, bin_number)
...
if __name__ == '__main__':
    app_flask.run(host='192.168.12.120', port=5000)
