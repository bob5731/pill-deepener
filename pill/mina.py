...
def initialize_database():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS pills
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, hour INTEGER, am_pm TEXT)''')
    conn.commit()
    conn.close()
...
def add_pill(name, hour, am_pm):
    if len(get_all_pills()) >= 30:
        ...
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("INSERT INTO pills (name, hour, am_pm) VALUES (?, ?, ?, ?)", (name, hour, am_pm))
        conn.commit()
        conn.close()
